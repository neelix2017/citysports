...
async def notify_(address,  debug=True):
    ...
    async with BleakClient(address, timeout  = 10) as client:
        ...
        while (Running):
            if len(queue) > 0:
                if (_time >= queue[0][0]):
                    logger.info("time: %s , set speed: %s", _time, queue[0][1])
                    _command = createCommand(queue[0][1])
                    if (_command != None):
                        await client.write_gatt_char("ffeeddcc-bbaa-9988-7766-554433221101",  bytearray(bytes.fromhex(_command)), response=False)
                    if (queue[0][1]=="stop"): 
                        Running = False
                    queue = queue[1:]
            _time += 1
